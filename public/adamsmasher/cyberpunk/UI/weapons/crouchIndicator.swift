...
public class CrouchIndicatorGameController extends inkHUDGameController {
  ...
  private final func RegisterBB() -> Void {
    ...
    if IsDefined(this.m_Player) && this.m_Player.IsControlledByLocalPeer() {
      ...
      this.OnPSMVisionStateChanged(playerSMBB.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Vision));
      this.OnPSMLocomotionStateChanged(playerSMBB.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Locomotion));
    };
  }
