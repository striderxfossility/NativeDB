...
public class SideScrollerMiniGameScoreSystem extends ScriptableSystem {
  ...
  private final func OnSendScore(request: ref<SendScoreRequest>) -> Void {
    let score: Int32;
    let id: Int32 = this.GetGameId(request.m_gameName);
    if id >= 0 {
      score = request.m_score;
